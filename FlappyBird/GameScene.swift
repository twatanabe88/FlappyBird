...
class GameScene: SKScene,SKPhysicsContactDelegate {
    ...
    var musicaction : SKAction = SKAction.playSoundFileNamed("warning1.mp3", waitForCompletion: true)
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        let bird_item = birdCategory | itemCategory
        ...
        else if (contact.bodyA.categoryBitMask | contact.bodyB.categoryBitMask) == bird_item || (contact.bodyB.categoryBitMask | contact.bodyA.categoryBitMask) == bird_item {
    ...
    func setupWall() {
        ...
        let itemTexture = SKTexture(imageNamed: "sushi")
        itemTexture.filteringMode = SKTextureFilteringMode.Nearest
        
        // スクロールするアクションを作成
        // 左方向に画像一枚分スクロールさせるアクション
        let moveItem = SKAction.moveByX(-(self.frame.size.width + itemTexture.size().width * 2) , y: 0, duration: 5.0)
        
        // 元の位置に戻すアクション
        let resetItem = SKAction.moveByX(self.frame.size.width + itemTexture.size().width * 2, y: 0, duration: 0.0)
        
        // 左にスクロール->元の位置->左にスクロールと無限に繰り替えるアクション
        let repeatScrollitem = SKAction.repeatActionForever(SKAction.sequence([moveItem, resetItem]))
        ...
        //                self.item.xScale = 0.03
        //                self.item.yScale = 0.03
        
        // スプライトの表示する位置を指定する
        item.position = CGPoint(x: self.frame.size.width + itemTexture.size().width * 2, y: wallTexture.size().height)
        
        
        // スプライトに物理演算を設定する
        item.physicsBody = SKPhysicsBody(circleOfRadius: itemTexture.size().height / 2.0)
        ...
        item.zPosition = -20.0
        item.runAction(repeatScrollitem)
        // スプライトを追加する
        ...
        // 次の壁作成までの待ち時間のアクションを作成
        let waitItemAnimation = SKAction.waitForDuration(2)
